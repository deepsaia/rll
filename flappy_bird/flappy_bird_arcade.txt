...
class FlappyBirdGame(arcade.Window):
    ...
    def _get_enhanced_observation(self):
        ...
        for pipe in sorted(self.game.pipes, key=lambda p: p.x):
            if pipe.x + pipe.width / 2 > self.game.bird.x:
                pipes_found.append(pipe)
        
        if len(pipes_found) >= 1:
            next_pipe = pipes_found[0]
        if len(pipes_found) >= 2:
            second_pipe = pipes_found[1]
        
        # First pipe information
        if next_pipe is None:
            pipe_x1 = self.game.bird.x + SCREEN_WIDTH
            gap_y1 = SCREEN_HEIGHT // 2
        else:
            pipe_x1 = next_pipe.x
            gap_y1 = next_pipe.gap_y
        
        # Second pipe information (look ahead)
        if second_pipe is None:
            pipe_x2 = pipe_x1 + 300  # Estimate position of next-next pipe
            gap_y2 = SCREEN_HEIGHT // 2
        else:
            pipe_x2 = second_pipe.x
            gap_y2 = second_pipe.gap_y
        
        # Calculate distances
        dist_to_pipe1 = pipe_x1 - self.game.bird.x
        dist_to_pipe2 = pipe_x2 - self.game.bird.x
        
        # Normalized positions
        bird_y_norm = self.game.bird.y / SCREEN_HEIGHT
        vel_norm = self.game.bird.velocity / 10  # Assuming max velocity of 10
        pipe1_gap_top_norm = (gap_y1 - PIPE_GAP/2) / SCREEN_HEIGHT
        pipe1_gap_bottom_norm = (gap_y1 + PIPE_GAP/2) / SCREEN_HEIGHT
        pipe2_gap_top_norm = (gap_y2 - PIPE_GAP/2) / SCREEN_HEIGHT
        pipe2_gap_bottom_norm = (gap_y2 + PIPE_GAP/2) / SCREEN_HEIGHT
        
        # Relative positions
        dist_to_pipe1_norm = dist_to_pipe1 / SCREEN_WIDTH
        dist_to_pipe2_norm = dist_to_pipe2 / SCREEN_WIDTH
        
        # Time since last jump (for temporal awareness)
        time_since_jump = getattr(self, 'time_since_jump', 100)
        if hasattr(self, 'prev_action') and self.prev_action == 1:
            time_since_jump = 0
        else:
            time_since_jump += 1
        self.time_since_jump = time_since_jump
        time_since_jump_norm = min(time_since_jump, 50) / 50
        
        return np.array([
            bird_y_norm,           # 0. Normalized bird y position
            vel_norm,              # 1. Normalized velocity
            dist_to_pipe1_norm,    # 2. Normalized distance to first pipe
            pipe1_gap_top_norm,    # 3. Normalized top of first pipe gap
            pipe1_gap_bottom_norm, # 4. Normalized bottom of first pipe gap
            dist_to_pipe2_norm,    # 5. Normalized distance to second pipe
            pipe2_gap_top_norm,    # 6. Normalized top of second pipe gap
            pipe2_gap_bottom_norm, # 7. Normalized bottom of second pipe gap
            time_since_jump_norm   # 8. Normalized time since last jump
        ], dtype=np.float32)
...
class FlappyBirdEnv(gym.Env):
    ...
    # Enhanced reward calculation in the step method
    def calculate_reward(self, prev_state, curr_state):
        """Calculate a more sophisticated reward."""
        # Base survival reward (decreasing over time to encourage progress)
        base_reward = 0.1
        
        # Distance-based reward - encourage moving forward
        distance_reward = (curr_state[2] - prev_state[2]) * 0.01  # Change in pipe distance
        
        # Height optimization - encourage staying in optimal flight corridor
        optimal_min = PIPE_GAP + FLOOR_HEIGHT
        optimal_max = SCREEN_HEIGHT - PIPE_GAP
        height_penalty = 0
        
        if curr_state[0] < optimal_min:  # Too low
            height_penalty = (optimal_min - curr_state[0]) * 0.005
        elif curr_state[0] > optimal_max:  # Too high
            height_penalty = (curr_state[0] - optimal_max) * 0.005
        
        # Velocity smoothing - penalize extreme velocity changes
        vel_penalty = abs(curr_state[1] - prev_state[1]) * 0.001
        
        # Pipe passage bonus (increased)
        pipe_bonus = 10.0 if self.just_passed_pipe else 0.0
        self.just_passed_pipe = False
        
        # Death penalty
        death_penalty = -20.0 if not curr_state['bird_alive'] else 0.0
        
        total_reward = (base_reward + distance_reward - height_penalty - 
                    vel_penalty + pipe_bonus + death_penalty)
